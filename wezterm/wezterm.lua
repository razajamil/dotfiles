...
-- Tab title: show index only, or index.user_title if manually renamed
wezterm.on("format-tab-title", function(tab)
	local index = tab.tab_index + 1
	local user_title = tab.tab_title

	if user_title and #user_title > 0 then
		return string.format(" %d.%s ", index, user_title)
	else
		return string.format(" %d ", index)
	end
end)

-- Show domain:workspace on the left of the tab bar
...
wezterm.on("user-var-changed", function(window, pane, name, value)
	if name == "workmux-switch-pane" then
		...
		window:perform_action(wezterm.action.SwitchToWorkspace({ name = data.workspace }), pane)
...
-- Connect to unix:core domain on startup
config.default_gui_startup_args = { "connect", "--workspace", "core", "unix" }
